...
class SynthTable:
    def __init__(self, crafter, synth_profit_threshold, storage_profit_threshold, sort_column, reverse_sort) -> None:
        ...
        self.recipes += [s.recipe for s in synths if s.can_craft and s.recipe not in self.recipes]
    ...
    def print(self):
        ...
        for recipe in self.recipes:
            ...
            synth.profit_per_synth, synth.profit_per_storage = synth.calculate_stats()

            meets_synth_profit = synth.profit_per_synth >= self.synth_profit_threshold
            meets_inventory_profit = synth.profit_per_storage >= self.storage_profit_threshold
            ...
            row = [synth.recipe.id, nq_name, nq_qty, hq1_name, hq1_qty, hq2_name, hq2_qty, hq3_name, hq3_qty,
                   synth.cost, synth.profit_per_synth, synth.profit_per_storage]

            # A row already exists in the list with the same synth results
            duplicate = [r for r in rows if r[1] == row[1] and r[3] == row[3] and r[5] == row[5] and r[7] == row[7]]
