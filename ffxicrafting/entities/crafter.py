...
class Crafter:
    ...
    def craft(self, item_controller):
        ...
        self.set_results_profit(results, simulation_cost, item_controller)
    ...
    def set_results_profit(self, results, simulation_cost, item_controller):
        for result, quantity in results.items():
            ...
            result.single_profit = self.get_single_profit(result, result.crafted_cost)
            result.stack_profit = self.get_stack_profit(result, result.crafted_cost)

    def get_single_profit(self, result, crafted_cost):
        return self.calculate_profit(result, crafted_cost, single=True)

    def get_stack_profit(self, result, crafted_cost):
        return self.calculate_profit(result, crafted_cost, single=False)

    def calculate_profit(self, result, crafted_cost, single=True):
    ...
    def get_profit_per_storage(self, results, simulation_cost):
        total_profit = self.calculate_total_profit(results, simulation_cost)
        total_storage = self.calculate_total_storage(results)
        profit_per_storage = total_profit / total_storage if total_storage > 0 else 0
        return profit_per_storage

    def calculate_total_profit(self, results, simulation_cost):
        total_gil = 0
        for result, quantity in results.items():
